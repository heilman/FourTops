...
//flag for scale sys
    cout <<"Scale sys up"<<endl;
    int doScaleShift = 1;


//flag for scale sys
    int doMatchingShift = 0;
...
// mistag scalefactor => TCHEL: data/MC scalefactor = 1.11 +- 0.12,    TCHEM: data/MC scalefactor = 1.21 +- 0.17
 ...
 // setTDRStyle();
  ...
  if(domisTagEffShift == 2)
    ...
    if (!sig_region)  postfix= postfix+"_Control";
...
//      xmlFileName = "config/test_fullsamples_el.xml";
  ...
  if (doScaleShift==1)  xmlFileName = "config/test_ScaleDown.xml";
  if (doScaleShift==2)  xmlFileName = "config/test_ScaleUp.xml";
  if (doMatchingShift==1)  xmlFileName = "config/test_MatchingDown.xml";
  if (doMatchingShift==2)  xmlFileName = "config/test_MatchingUp.xml";
...
////////////////////////////////
 ...
 //   MVAvars.push_back("PTBalTopEventX");
  ...
  cout << "Rescaling to an integrated luminosity of "<< Luminosity <<" pb^-1" << endl;
    ...
    // frac is the factor by which the TTJets sample, it is minimally 1.4
    double frac = 1.4;
    double currentLumi = datasets[ndatasets]->EquivalentLumi();
    cout <<"Old lumi =   "<< currentLumi  <<endl;
    double newlumi = currentLumi/frac;
    datasets[ndatasets]->SetEquivalentLuminosity(newlumi);
  ...
  TRootEvent* event = 0;
   ...
   // MSPlot["NbOfIsolatedElectrons"] = new MultiSamplePlot(datasets, "NbOfIsolatedElectron", 10, 0, 10, "Nb. of isolated electrons");
  ...
  ///////////////////
   ...
   // histo2D["Corrections_nJetsvnTags"] = new TH2F(("Corrections_nJetsvnTags").c_str(),"nJets:nTags",9,-0.5,8.5, 16, -0.5,15.5);
  ...
  for (unsigned int d = 0; d < datasets.size(); d++){
      ...
      histo2D[("nJetsvnTags"+datasets[d]->Name()).c_str()] = new TH2F(("Corrections_nJetsvnTags"+datasets[d]->Name()).c_str(),"nJets:nTags",9,-0.5,8.5, 16, -0.5,15.5);
  ...
  //  CutsSelecTableMu.push_back(string("PU reweighting"));
    ...
    CutsSelecTableMu.push_back(string("Veto on extra loose muons"));
    CutsSelecTableMu.push_back(string("Veto on loose electrons"));
    CutsSelecTableMu.push_back(string("$>$= 6 Jets (40 GeV)"));
    CutsSelecTableMu.push_back(string("$>$= 2 CSVM Tags"));
    CutsSelecTableMu.push_back(string("HT $>$=  400 GeV"));
...
//////////////////////////////////////////////////
    ...
    JetCorrectorParameters *L1JetPar  = new JetCorrectorParameters("../../TopTreeAnalysisBase/Calibrations/JECFiles/START42_V17_AK5PFchs_L1FastJet.txt");
    JetCorrectorParameters *L2JetPar  = new JetCorrectorParameters("../../TopTreeAnalysisBase/Calibrations/JECFiles/START42_V17_AK5PFchs_L2Relative.txt");
    JetCorrectorParameters *L3JetPar  = new JetCorrectorParameters("../../TopTreeAnalysisBase/Calibrations/JECFiles/START42_V17_AK5PFchs_L3Absolute.txt");
    ...
    if(dataSetName == "Data" || dataSetName == "data" || dataSetName == "DATA") // Data!
      {
	JetCorrectorParameters *ResJetCorPar = new JetCorrectorParameters("../../TopTreeAnalysisBase/Calibrations/JECFiles/START42_V17_AK5PFchs_L2L3Residual.txt");
	vCorrParam.push_back(*ResJetCorPar);
      }
    
    JetCorrectionUncertainty *jecUnc = new JetCorrectionUncertainty("../../TopTreeAnalysisBase/Calibrations/JECFiles/START42_V17_AK5PFchs_Uncertainty.txt");
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	if (debug) cout<<" jet1 pt =  "<<selectedJets[0]->Pt() << "   "<< " jet2 pt =  "<<selectedJets[1]->Pt() << "   "<< " jet2 pt =  "<<selectedJets[2]->Pt() << "   "<< " jet3 pt =  "<<selectedJets[3]->Pt() << "  JetCut?"  << JetCut  <<endl;
        ...
        int applyBTagSF = 1; // 1 = JetByJet Upgrading/Downgrading, 2 = EventWeigthing using SF and MC efficiencies.
        
if (applyBTagSF ==1){
            
    if (debug)cout <<"applying jet upgrading SF"<<endl;
    ...
    for ( seljet =0; seljet < selectedJets.size(); seljet++ ){
        ...
        if(dataSetName!="Data"){
            ...
            if( selectedJets[seljet]->btag_combinedSecondaryVertexBJetTags() > workingpointvalue  ){
                ...
                double BTageff   ;
                double LightJeteff;
                double JetPt = selectedJets[seljet]->Pt() ;
                BTagSFUtil* btsfutil = new BTagSFUtil( seed );

                    if(BTagGraph_L_eta008){
                   
                        BTageff =  BTagGraph_B->Eval(JetPt,0,"");
                        
                        if(fabs(selectedJets[seljet]->Eta()) < 0.8) LightJeteff =  BTagGraph_L_eta008->Eval(JetPt,0,"");
                        else if(fabs(selectedJets[seljet]->Eta()) >= 0.8  &&  fabs(selectedJets[seljet]->Eta()) < 1.6 ) LightJeteff =  BTagGraph_L_eta0816->Eval(JetPt,0,"");
                        else if(fabs(selectedJets[seljet]->Eta()) >= 1.6 &&  fabs(selectedJets[seljet]->Eta()) <= 2.4) LightJeteff =  BTagGraph_L_eta1624->Eval(JetPt,0,"");
          ...
          double bTagSF_b = ( bTool->getWeight(selectedJets[seljet]->Pt(), selectedJets[seljet]->Eta(), 5,0 ));
          double bTagSF_c = ( bTool->getWeight(selectedJets[seljet]->Pt(), selectedJets[seljet]->Eta(), 4,0 ));
          double bTagSF_l = ( bTool->getWeight(selectedJets[seljet]->Pt(), selectedJets[seljet]->Eta(), 1,0 ));
     ...
     if (selectedJets[seljet]->btag_combinedSecondaryVertexBJetTags() > workingpointvalue   ){
        ...
        double njets = selectedJets.size();
        double ntags = selectedBJets.size();
      //  if (dataSetName=="Data") {
      //      corr_scale = 1.;
      //  }else{
      //      corr_scale = ;
      //  }
        ...
        //////////////////////////////////////////////////////////////////////
        ////                                                            //////
        ////  Applying scale factors to correct for disagreement at     //////
        ////   large number of jets/tags.                               //////
        ////                                                            //////
        //////////////////////////////////////////////////////////////////////
        //Tags:      2,3,4,5
        //Jets:      6,7,8,9
        double corr_factors[4][4] = {{1.07, 1.12, 1.59, 16.9},{1.28, 1.17,.30,1.},{1.87,1.62,4.22,1.},{2.81,1.,1.,1.}};
        int corr_index_j;
        int corr_index_t;
        bool applyjettag_corr = false;
        if(dataSetName == "Data" || dataSetName == "NP_overlay_TTTT") applyjettag_corr = false;
        int nj = njets - 6;
        int nt = ntags - 2;
        if(applyjettag_corr){
            
            if(njets  < 8 ){   scaleFactor = scaleFactor*corr_factors[nt][nj];
            }else{
            
            scaleFactor = scaleFactor*corr_factors[nt][2];
       ...
       /////////////////////
        ...
        if (nMu==1){  selecTableMu.Fill(d,2,1.);
            ...
            if (selectedJets.size() >= 6) {  selecTableMu.Fill(d,5,1.) ;
                if (selectedBJets.size() >= 2){ selecTableMu.Fill(d,6,1.) ;
                    if (temp_HT >= 400.){ selecTableMu.Fill(d,7,1.) ;
                    }
                }
            }
                }
        ...
        if (debug)cout <<"cutflow written"<<endl;
...
if  (  !( selectedBJets.size() >= 2  &&  nMu == 1 && selectedLooseMuons.size() ==1 && selectedLooseElectrons.size()   ==0 && temp_HT > 400.)) continue;
 passed++;
 cout <<"Eventnnnnn passed..."<< passed  <<endl;
        
        
 //  histo2D[("nJetsvnTags"+datasets[d]->Name()).c_str()]->Fill(ntags,njets, datasets[d]->NormFactor()*Luminosity);
        
       // if(njets == 6 || njets == 7){
       //  histo2D[("nJetsvnTags"+datasets[d]->Name()).c_str()]->Fill(ntags,njets, datasets[d]->NormFactor()*Luminosity);
       // }else{
       // histo2D[("nJetsvnTags"+datasets[d]->Name()).c_str()]->Fill(ntags,8, datasets[d]->NormFactor()*Luminosity);
//        }
            
            
//if  (  !( sig_region && selectedBJets.size() >= 2  &&  nMu == 1 && selectedLooseMuons.size() ==1 && selectedLooseElectrons.size()   ==0 && temp_HT < 400.)) continue;
...
if(!TrainMVA){
    ...
    MSPlot["MVA2ndPassTriJet"]->Fill(MultiTopness,  datasets[d], true, Luminosity*scaleFactor );
...
//////////////////////////////////////////////////////////////////////////////////
  ...
  for (Int_t b = minNJets; b <= maxNJets; b++){
	  ...
	  //	MSPlot["1stLeadingElectronRelIsolation"]->Fill(selectedElectrons_NoIso[0]->relativePfIso(), datasets[d], true, Luminosity*scaleFactor);
        cout <<"electron channel..."<< selectedElectrons.size()<<"  "<< dataSetName <<endl;
    
// cout <<"Number of Muons, Jets, BJets, JetCut  ===>  "<< selectedMuons.size() <<"  "  << selectedJets.size()  <<endl;
        
    MSPlot["NbOfIsolatedElectrons"]->Fill(selectedElectrons.size(), datasets[d], true, Luminosity*scaleFactor);
    MSPlot["NbOfIsolatedMuons"]->Fill(selectedMuons.size(), datasets[d], true, Luminosity*scaleFactor);
        //cout <<"electron channel..."<< selectedElectrons.size()<<endl;
  ...
  //Options : WithError (false), writeMerged (true), useBookTabs (false), addRawNumbers (false), addEfficiencies (false), addTotalEfficiencies (false), writeLandscape (false)
  	selecTableMu.Write(  "FourTop"+postfix+"Table_Mu.tex",    true,true,true,true,false,false,false);
  }
    else if(Electron){
	//(bool mergeTT, bool mergeQCD, bool mergeW, bool mergeZ, bool mergeST)
  ...
  for(map<string,MultiSamplePlot*>::const_iterator it = MSPlot.begin(); it != MSPlot.end(); it++)
    {
      ...
      //TFile * tempErrorFile;
        if(doScaleShift == 1){
            ...
            string filename = "ScaleFiles/Error_"+name+".root";
...
temp->Draw_wSysUnc(false, name, true, true, true, true, true,100.,true); // merge TT/QCD/W/Z/ST/
  ...
  for(map<std::string,TH1F*>::const_iterator it = histo1D.begin(); it != histo1D.end(); it++)
  {
    ...
    double ttjets_exp = 1;
    double data_obs = 1;
    //Fill JetsVsTags correction 2D histo
  ...
  th2dir->cd();
   ...
   // TH2F *rat = histo2D["nJetsvnTagsData"];
    //TH2F rat_clone = rat->Clone();
    
    TH2F *rat=(TH2F*)histo2D["nJetsvnTagsData"]->Clone();
    rat->SetName("Ratio_DATA_TTJets");
    rat->Divide(histo2D["nJetsvnTagsTTJets"]);
    rat->Write();
   ...
   for(map<std::string,TH2F*>::const_iterator it = histo2D.begin(); it != histo2D.end(); it++)
  {
    ...
    cout <<"Post processing 2D plots..."<< endl;
    //TH2F *hRat2d =  histo2D["nJetsvnTagsData"]->Clone();
    //hRat2d->Write();
