...
////////////////////////////////
 ...
 //  Eventtrainer_->bookInputVar("bTagRatio14");
  ...
  TRootEvent* event = 0;
    ...
    MSPlot["ElectronPt"]              = new MultiSamplePlot(datasets, "ElectronPt", 35, 0, 300, "PT_{e}");
    ...
    MSPlot["ElectronMVATrigID"]              = new MultiSamplePlot(datasets, "ElectronMVATrigID", 70, -1,    1, "MVA Trig ID");
    ...
    MSPlot["MultiTopness"] = new MultiSamplePlot(datasets, "MultiTopness", 35, -1., 0, "MultiTopness");
...
MSPlot["HTRat"]                  = new MultiSamplePlot(datasets, "HTRat", 50, 0, 20, "HTRat");
 ...
 histo1D["btag_weight"] = new TH1F("btag_weight","btag_weight;btag_weight;#events",150,-10,10);  
  ...
  CutsSelecTableEl.push_back(string("$>$= 6 Jets (30 GeV)"));
  CutsSelecTableEl.push_back(string("$>$= 2 CSVM Tags"));
  CutsSelecTableEl.push_back(string("HT $>$=  400 GeV"));
  CutsSelecTableEl.push_back(string("MET > 30 GeV"));
...
double bTagSF_l = (bTool->getWeight(selectedJets[seljet]->Pt(),selectedJets[seljet]->Eta(), 1,domisTagEffShift ));
 ...
 if (event_weight > 0. && event_weight < 10.) scaleFactor = scaleFactor*event_weight; 
...
HT_lagging += selectedJets[seljet0]->Pt();
   ...
   double HTb = 0.;
	  ...
	  //Apply the lepton, btag and HT selections
        if  (  !( selectedBJets.size() >= 2 && selectedElectrons.size() == 1 && selectedLooseElectrons.size() ==1 && selectedLooseMuons.size() ==0 && temp_HT >= 400. && mets[0]->Et()> 30. )) continue;
...
////////////////////////////////////////////////////////////////////////////////////
       ...
       double reliso = (selectedElectrons[0]->chargedHadronIso()+selectedElectrons[0]->neutralHadronIso()+selectedElectrons[0]->photonIso())/selectedElectrons[0]->Pt();
...
//   TFile* sysFile = new TFile("SystematicShapes.root","RECREATE");
  ...
  for(map<string,MultiSamplePlot*>::const_iterator it = MSPlot.begin(); it != MSPlot.end(); it++)
    {
	  ...
	  cout <<"    "<<endl;
	    ...
	    delete tempErrorFile;
            ...
            tempErrorFile->Close();
          if(name=="MVA"){
             TFile* sysFile = new TFile("SystematicShapes.root","UPDATE");
 ...
 if(name=="MVA"){
   ...
   // temp->addText("CMS preliminary");
	    temp->Draw_wSysUnc(false, name, false, true, true, true, true,100.,true, true, true); // merge TT/QCD/W/Z/ST/
