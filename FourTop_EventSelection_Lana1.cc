...
int main (int argc, char *argv[])
{
  ...
  string overridexmlfile="";
  bool doXMLoverride=false;

  bool doPDFs=false;
  ofstream pdfinfofile;
  string pdffilename="PDFs_mu.txt";
  ...
  if(argc>0){
	...
	val=argv[iarg];
	if(val.find(".xml")!=std::string::npos){
	  overridexmlfile=argv[iarg];
	  doXMLoverride=true;
	  cout << "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!" << endl;
	  cout << "now overriding XML file, using " << overridexmlfile << " instead!!!" << endl;
	}
      }
    }
    ...
    while(iarg<argc){
      string val = argv[iarg];
      iarg++;
      if(val.find("--pdf")!=std::string::npos){
	doPDFs=true;
	cout << "!!!!!!! doing PDF uncertainties!!!" << endl;

      }
    }
  }
  if(doPDFs)
    pdfinfofile.open(pdffilename.c_str());
...
// mistag scalefactor => TCHEL: data/MC scalefactor = 1.11 +- 0.12,    TCHEM: data/MC scalefactor = 1.21 +- 0.17
 ...
 // setTDRStyle();
  ...
  string postfix = "_EventSelection_NEW"; // to relabel the names of the output file
...
//      xmlFileName = "config/test_fullsamples_el.xml";
 ...
 else if(doXMLoverride){
   xmlFileName=overridexmlfile;
 }
  else{
       xmlFileName = "config/test_TuneP11_mu_rereco.xml";
       // xmlFileName = "config/test_mconly.xml";
  }
...
////////////////////////////////
  ...
  float Luminosity = 19695.022;  //18848.367; //pb^-1??
  ...
  JetCombiner* jetCombiner = new JetCombiner(TrainMVA, Luminosity, datasets, MVAmethod, false);
    ...
    //uncomment these 2 lines for training
 ...
 //histo1D["MVA_Nominal_Tune"] = new TH1F(datasets, "MVA", 15, -0.3, 0.4, "BDT Discriminator"); 
...
//Apply the lepton, btag and HT selections
   if  (  !( nMu == 1 && selectedLooseMuons.size() ==1 && selectedBJets.size() >= 2    &&selectedLooseElectrons.size()   ==0 && temp_HT > 400. && mets[0]->Et() > 30.    )) continue;
 passed++;
...
////////////////////////////////////////////////////////////////////////
	...
	histoProfile[("HTX_vs_MultiTopness_"+datasets[d]->Name()).c_str()]->Fill(MultiTopness, HTX);
        ...
        histoProfile[("HTX_vs_NbOfSelectedBJets_"+datasets[d]->Name()).c_str()]->Fill(selectedBJets.size(),HTX);        
        ...
        histoProfile[("MultiTopness_vs_NbOfSelectedBJets_"+datasets[d]->Name()).c_str()]->Fill(selectedBJets.size(),MultiTopness);
        ...
        histoProfile[("HTH_vs_NbOfSelectedBJets_"+datasets[d]->Name()).c_str()]->Fill(selectedBJets.size(),HTH);
        ...
        histoProfile[("5thJetPt_vs_NbOfSelectedBJets_"+datasets[d]->Name()).c_str()]->Fill(selectedBJets.size() ,selectedJets[4]->Pt());
        ...
        histoProfile[("6thJetPt_vs_NbOfSelectedBJets_"+datasets[d]->Name()).c_str()]->Fill(selectedBJets.size(),selectedJets[5]->Pt());
        ...
        histoProfile[("HTb_vs_NbOfSelectedBJets_"+datasets[d]->Name()).c_str()]->Fill(selectedBJets.size(),HTb);
        ...
        histoProfile[("HTRat_vs_NbOfSelectedBJets_"+datasets[d]->Name()).c_str()]->Fill(selectedBJets.size(),HTRat);
        histoProfile[("HTRat_vs_SumJetMassX_"+datasets[d]->Name()).c_str()]->Fill(SumJetMassX,HTRat);

        histoProfile[("NbOfSelectedJets_vs_NbOfSelectedBJets_"+datasets[d]->Name()).c_str()]->Fill(selectedBJets.size(),selectedJets.size());
        histoProfile[("NbOfSelectedJets_vs_SumJetMassX_"+datasets[d]->Name()).c_str()]->Fill(SumJetMassX,selectedJets.size());

        histoProfile[("NbOfSelectedBJets_vs_SumJetMassX_"+datasets[d]->Name()).c_str()]->Fill(SumJetMassX,selectedBJets.size());
	...
	cout <<"njets  "<< selectedJets.size()  << "nTags " <<  selectedBJets.size()     <<    " 5th jet pt "     << selectedJets[4]->Pt() <<  " 6th jet pt "     << selectedJets[5]->Pt() <<" multitopness " << MultiTopness    <<  " zumjetmassX " << SumJetMassX    <<  " HTRat" << HTRat<< "  HTb" << HTb << "  HTH " << HTH<< " HTX"  << HTX  <<endl;
	...
	//histo1D["MVA_Nominal_Tune"]->Fill(BDTscore, datasets[d], true, Luminosity*scaleFactor);
       
	if((BDTscore > 0.2) && (dataSetName=="Data")){
	  eventlist <<currentRun  << " " << event->lumiBlockId() <<" " <<event->eventId() << " "  << BDTscore <<" N Jets = " << selectedJets.size()<<" N Tags = " << selectedBJets.size()<< " MultiTopness => "  <<MultiTopness<< " HTX = >  "<<HTX<< " HTRat = > "<< HTRat << " HTb =>  "<< HTb  << endl;        

	}


	// getting out pdf information
	//cout << "PDF input info: "  << event->idParton1() << " " << event->idParton2() << " " << event->xParton1() << " " << event->xParton2() << " " << event->factorizationScale() << endl;
	if(doPDFs){
	  if(pdfinfofile.is_open()){
	    
	    
	    pdfinfofile  << event->idParton1() << " " << event->idParton2() << " " << event->xParton1() << " " << event->xParton2() << " " << event->factorizationScale() << " " << /*Luminosity**/scaleFactor << " " << BDTscore << endl;
	  }
	}
