...
void rf801_mcstudy()
{
 ...
 RooRealVar* x = w->var("obs_x_channel_mu") ;

 // RooRealVar x("x","x",-0.3,0.4) ;
 // x.setBins(15) ;

  w->Print();
  RooAbsPdf* model_mu = w->pdf("model_channel_mu");
  RooAbsPdf* model_el = w->pdf("model_channel_el");
  RooDataSet* data_mu = w->data("obs_x_channel_mu");

  model_mu->Print("t") ;
 
  //  RooDataSet * data = pdf->generate( RooArgSet(*x,*index), AllBinned());

  //  RooDataSet *data = w->Get("obsData") ;

   model_mu->fitTo(*data_mu);

    RooPlot* frame = x->frame() ;
   model_mu->plotOn(frame) ;
  ...
  // to speed up the study at the expemse of some precision
   ...
   //  RooMCStudy* mcstudy = new RooMCStudy(model_mu,x,Binned(kTRUE),Silence(),Extended(), FitOptions(Save(kTRUE),PrintEvalErrors(0))) ;
