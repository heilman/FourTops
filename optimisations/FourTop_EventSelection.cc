...
// mistag scalefactor => TCHEL: data/MC scalefactor = 1.11 +- 0.12,    TCHEM: data/MC scalefactor = 1.21 +- 0.17
 ...
 // setTDRStyle();
  ...
  string postfix = "_EventSelection_wMETCut"; // to relabel the names of the output file
...
////////////////////////////////
  ...
  JetCombiner* jetCombiner = new JetCombiner(TrainMVA, Luminosity, datasets, MVAmethod, false);
    ...
    // MVATrainer* Eventtrainer_ = new MVATrainer("BDT","MasterMVA_Mu", "MasterMVA_Mu.root");
 ...
 //   MVAvars.push_back("PTBalTopEventX");
  ...
  //  MVAvars.push_back("Jet4Pt");
   ...
   // MVAComputer* Eventcomputer_ = new MVAComputer("BDT","MasterMVA.root","MasterMVA",MVAvars, "test");
    ...
    MVAComputer* Eventcomputer_ = new MVAComputer("BDT","MasterMVA_Mu.root","MasterMVA_Mu",MVAvars, "test");
 ...
 MSPlot["MVA1TriJet_3"] = new MultiSamplePlot(datasets, "MVA1TriJet_3", 35, -1., 0.5, "BDT Discriminator");
   ...
   // MSPlot["NbOfIsolatedElectrons"] = new MultiSamplePlot(datasets, "NbOfIsolatedElectron", 10, 0, 10, "Nb. of isolated electrons");
    ...
    Int_t minNJets=6, maxNJets=9, minNBJets=2, maxNBJets=4;
    Int_t q =0;
    ...
    MSPlot[MVA_Name.c_str()] = new MultiSamplePlot(datasets, MVA_Name.c_str(), 15, -0.3, 0.4, "BDT Disciminator");
...
///////////////////////////////////////////////////////////
    ...
    int applyBTagSF = 2; // 1 = JetByJet Upgrading/Downgrading, 2 = EventWeigthing using SF and MC efficiencies.
    ...
    for ( seljet =0; seljet < selectedJets.size(); seljet++ ){
        ...
        if(dataSetName!="Data"){
            ...
            if( selectedJets[seljet]->btag_combinedSecondaryVertexBJetTags() > workingpointvalue  ){
		      ...
		      //   cout<<"extracting eff curves"<<endl;
...
double bTagSF_b = ( bTool->getWeight(selectedJets[seljet]->Pt(),selectedJets[seljet]->Eta(),5,dobTagEffShift ));
double bTagSF_c = ( bTool->getWeight(selectedJets[seljet]->Pt(), selectedJets[seljet]->Eta(), 4,dobTagEffShift ));
double bTagSF_l = (bTool->getWeight(selectedJets[seljet]->Pt(),selectedJets[seljet]->Eta(), 1,domisTagEffShift ));
 bool istagged_bf = isTagged;

 // cout <<"before modification =  "<< isTagged<<endl;
          btsfutil->modifyBTagsWithSF(isTagged, jet_flavor, bTagSF_b, BTageff, bTagSF_l, LightJeteff);
	  delete btsfutil;
	  //  if(istagged_bf != isTagged) cout <<" modified! =  "<<endl;            
	  //cout <<"  "<<endl;
...
event_weight = (p_data)/(p_mc);
 ...
 if(event_weight < 0.){
   ...
   cout <<"NEGATIVE EVENT WEIGHT FROM BTAGGING!!!!! MUST BE FIXED!"<<endl;
	...
	if (debug)cout<<"applying event weight... " <<endl; 
...
//Apply the lepton, btag and HT selections
   if  (  !( selectedBJets.size() >= 2  &&  nMu == 1 && selectedLooseMuons.size() ==1 && selectedLooseElectrons.size()   ==0 && temp_HT > 400. && mets[0]->Et() > 30.    )) continue;
 passed++;
...
////////////////////////////////////////////////////////////////////////
	...
	// MSPlot["DeltaPhi_LepTop"]->Fill(ang_leptop,datasets[d], true, Luminosity*scaleFactor);
	  ...
	  //	  cout <<"tree weight..."<<  scaleFactor <<endl;
  ...
  for(map<string,MultiSamplePlot*>::const_iterator it = MSPlot.begin(); it != MSPlot.end(); it++)
    {
      ...
      //TFile * tempErrorFile;
        if(doScaleShift == 1){
            ...
            tempErrorFile->Close();
            if(name=="MVA"){
             TFile* sysFile = new TFile("SystematicShapes_Mu.root","RECREATE");
             tempHisto->Write("Scale_Down");
	    }
